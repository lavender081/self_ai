...
- 核心履历与重点项目：
  1. 杭州基础创新科技（2025.04-2026.04）| 产品经理：主导新一代 EMS 综合能源管理平台 0-1 架构与落地；构建 AIoT 物联底座，打通“监测-计量-计费-管控”闭环；支撑核心大客户攻坚，探索虚拟电厂演进。
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const payload = {
        model: 'qwen-max', // 建议升级模型（若 DashScope 账号支持），以获得更严格的指令遵从度；若原有的 qwen3-vl-flash 是业务强制要求可改回
        messages:[
          { role: 'system', content: SYSTEM_PROMPT },
          { role: 'user', content: message }
        ],
        temperature: 0.3, // 降低温度值，减少AI自由发散，确保内容更加严谨
        top_p: 0.8
      };
